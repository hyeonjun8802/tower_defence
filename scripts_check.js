...
function renderOfflineMetaPanel(){
  ...
  const menu = $('offlineMenuPanel'); if(menu) menu.innerHTML = html;
...
const CELL = IS_MOBILE_BOARD ? 72 : 62;
const GX = Math.round((W - GRID*CELL)/2);
const GY = IS_MOBILE_BOARD ? 64 : 120;
const CORE = {
...
class Planet{
  ...
  stats(){
    ...
    const plateDamage = b.plate ? g.plateDamage : 0;
    const plateFireRate = b.plate ? g.plateFireRate : 0;
...
function drawRouteChevron(x, y, angle, size, color, alpha=.34){
  ...
  ctx.lineWidth = 1.6;
  ctx.beginPath();
  ctx.moveTo(-size * .55, -size * .35);
  ctx.lineTo(size * .1, 0);
  ctx.lineTo(-size * .55, size * .35);
  ctx.stroke();
  ctx.restore();
}

function drawRouteSegmentMarkers(){
  const pulse = .18 + Math.sin(performance.now() / 520) * .03;
  for(let i=0;i<route.length-1;i++){
    ...
    for(let j=1;j<=count;j++){
      ...
      const x = a.x + dx * t;
      const y = a.y + dy * t;
      drawRouteChevron(x, y, angle, 10, '#f8fafc', pulse);
    }
    ctx.save();
    ctx.globalAlpha = .11;
    ctx.fillStyle = theme().color;
...
function drawRoute(){
  ...
  ctx.shadowBlur=9;
  ctx.strokeStyle='rgba(3,7,18,.74)';ctx.lineWidth=42;
  ctx.beginPath();ctx.moveTo(route[0].x,route[0].y);
  for(let i=1;i<route.length;i++)ctx.lineTo(route[i].x,route[i].y);
  ctx.stroke();

  ctx.shadowBlur=0;
  ctx.strokeStyle='rgba(148,163,184,.11)';ctx.lineWidth=38;
  ctx.beginPath();ctx.moveTo(route[0].x,route[0].y);
  for(let i=1;i<route.length;i++)ctx.lineTo(route[i].x,route[i].y);
  ctx.stroke();

  const pulse = .08 + Math.sin(performance.now()/560)*.015;
  ctx.strokeStyle=theme().color;ctx.globalAlpha=pulse;ctx.lineWidth=23;
  ctx.beginPath();ctx.moveTo(route[0].x,route[0].y);
  for(let i=1;i<route.length;i++)ctx.lineTo(route[i].x,route[i].y);
  ctx.stroke();

  ctx.globalAlpha=.09;ctx.strokeStyle='rgba(255,255,255,.82)';ctx.lineWidth=14;
  ctx.beginPath();ctx.moveTo(route[0].x,route[0].y);
  for(let i=1;i<route.length;i++)ctx.lineTo(route[i].x,route[i].y);
  ctx.stroke();

  ctx.setLineDash([8,16]);ctx.globalAlpha=.22;ctx.strokeStyle='rgba(226,232,240,.62)';ctx.lineWidth=1.8;
  ctx.beginPath();ctx.moveTo(route[0].x,route[0].y);
  for(let i=1;i<route.length;i++)ctx.lineTo(route[i].x,route[i].y);
  ctx.stroke();
  ctx.setLineDash([]);
...
function drawTerrain(){
  ctx.save();
  for(let i=0;i<terrain.length;i++){
    const c=center(i), t=TERRAIN[terrain[i]];
    ctx.fillStyle=t.color;
    ...
    if(terrain[i]!=='empty'&&terrain[i]!=='path'){
      ctx.fillStyle=terrain[i]==='blocked'?'#475569':theme().color;
      ctx.font='900 16px Orbitron';ctx.textAlign='center';ctx.textBaseline='middle';
      const s={blocked:'×',amp:'▲',coil:'»',lens:'◇',mine:'◆',rift:'!'}[terrain[i]];
...
function updateUI(){
  ...
  $('summonBtn').textContent=`랜덤 소환 ${fmt2(currentSummonCost())}`;
...
function updateSelected(){
  ...
  box.innerHTML=`<div class="selectedCompactHead">
    ...
    <div class="selectedCompactStats">공격 ${fmt2(st.dmg)} · 사거리 ${fmt2(st.range)}${critText}<br>장판 ${tr.name} · ${escapeHtml(towerSkillCompactText(t))}</div>`;
  updateHangarVisuals(true);
  updateHangarState();
}
function log(t){
...
(function(){
  ...
  const btn = $('towerMenuBtn');
  const closeBtn = $('towerPopupClose');
  const hangar = $('hangar');
  const legacyModal = $('planetDetailModal');
  const legacyBody = $('planetDetailBody');
  if(!popup || !list || !detail || !btn || !hangar) return;
  ...
  function htmlFrom(card, selector, fallback=''){
    const node = card?.querySelector(selector);
    return node ? node.innerHTML : fallback;
  }
  function getCards(){ return Array.from(hangar.querySelectorAll('.planetCard')); }
  function thumbHtml(card){
    const thumb = card?.querySelector('.planetThumb');
    if(!thumb) return '<div class="planetThumb"></div>';
    const clone = thumb.cloneNode(true);
    clone.classList.remove('big');
    return clone.outerHTML;
  }
  function getInfo(card){
    const name = textFrom(card,'.planetCardName', textFrom(card,'.towerName','타워'));
    const roleHtml = htmlFrom(card,'.planetCardRole', '전투 타워');
    const roleText = safeText(roleHtml.replace(/<[^>]+>/g,' '));
    const cost = textFrom(card,'.planetCardCost','COST -');
    const level = textFrom(card,'.planetCardLevel','LV.1');
    const lockTag = textFrom(card,'.lockTag','');
    const locked = card.classList.contains('locked') || /미해금|locked|lock/i.test(`${lockTag} ${cost} ${roleText}`);
    return {name, roleHtml, roleText, cost, level, lockTag, locked};
  }
  ...
  function buildDetailFromLegacy(card, info){
    let legacyHtml = '';
    try{
      card.click();
      legacyHtml = legacyBody && legacyBody.innerHTML ? legacyBody.innerHTML : '';
    }catch(err){
      console.warn('legacy planet detail copy failed', err);
    }
    closeLegacyModal();

    if(legacyHtml){
      detail.innerHTML = legacyHtml;
      const copiedThumbs = detail.querySelectorAll('.liveThumb');
      copiedThumbs.forEach((thumb) => {
  ...
  function buildFallbackDetail(card, info){
    detail.innerHTML = `
      <div class="towerDetailHero">
        <div class="towerDetailThumb">${thumbHtml(card)}</div>
        <div>
          <div class="towerDetailKicker">${info.locked ? 'LOCKED TOWER' : 'TOWER PROFILE'}</div>
          <div class="towerDetailName">${esc(info.name)}</div>
          <div class="towerDetailRole">${esc(info.roleText || '전투 중 배치 가능한 행성 타워입니다.')}</div>
        </div>
      </div>
      <div class="towerDetailGrid">
        <div class="towerDetailStat"><small>상태</small><b>${info.locked ? 'LOCKED' : 'READY'}</b></div>
        <div class="towerDetailStat"><small>비용</small><b>${esc(info.cost || '-')}</b></div>
        <div class="towerDetailStat"><small>레벨</small><b>${esc(info.level || '-')}</b></div>
      </div>
      <div class="towerPopupEmpty" style="margin-top:14px;">기존 상세 데이터를 불러오지 못해 기본 정보만 표시합니다.</div>`;
  }

  function buildList(){
    const cards = getCards();
    if(!cards.length){
      list.innerHTML = '<div class="towerPopupEmpty">표시할 타워가 아직 없습니다.</div>';
      detail.innerHTML = '<div class="towerPopupEmpty">전투가 시작되면 타워 목록이 표시됩니다.</div>';
      return;
    }
    ...
    cards.forEach((card, idx) => {
      ...
      item.type = 'button';
      item.className = 'towerPopupItem' + (idx === selectedIndex ? ' active' : '') + (info.locked ? ' locked' : '');
      item.innerHTML = `
        <div class="towerPopupThumb">${thumbHtml(card)}</div>
        <div class="towerPopupItemText">
          <div class="towerPopupItemName">${esc(info.name)}</div>
          <div class="towerPopupItemSub">${esc(info.roleText || info.cost)}</div>
        </div>
        <div class="towerPopupItemLv">${esc(info.level || info.cost)}</div>`;
      item.addEventListener('click', () => selectTower(idx));
      list.appendChild(item);
    });
    selectTower(selectedIndex, false);
  }

  function selectTower(index, syncList=true){
    const cards = getCards();
    const card = cards[index];
    if(!card) return;
    const sameSelection = index === selectedIndex;
    const keepLevelTableOpen = sameSelection && !!detail.querySelector('.planetDetailDetails[open]');
    const keepScrollTop = sameSelection ? (detail.scrollTop || 0) : 0;
    selectedIndex = index;
    if(syncList){
      Array.from(list.children).forEach((el, i) => el.classList.toggle('active', i === index));
    }
    const info = getInfo(card);
    if(!buildDetailFromLegacy(card, info)){
      buildFallbackDetail(card, info);
    }
    if(keepLevelTableOpen){
      const levelDetails = detail.querySelector('.planetDetailDetails');
      if(levelDetails) levelDetails.open = true;
      requestAnimationFrame(() => { detail.scrollTop = keepScrollTop; });
    }
  }
  ...
  new MutationObserver(() => {
    ...
    // 레벨 성장표를 펼친 상태에서는 자동 갱신으로 상세 HTML을 다시 그리지 않는다.
    // 기존에는 전투 중 hangar DOM 갱신이 들어오면서 details가 몇 초 뒤 다시 접혔다.
    if(detail.querySelector('.planetDetailDetails[open]')) return;
    clearTimeout(popupRefreshTimer);
    popupRefreshTimer = setTimeout(buildList, 120);
  }).observe(hangar, {childList:true, subtree:true, attributes:true, attributeFilter:['class','style','data-level']});
})();
