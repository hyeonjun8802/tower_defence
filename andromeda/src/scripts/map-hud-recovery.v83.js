...
(function(){
  ...
  function openInfo(kind){
    ...
    var enter = byId('v274MapInfoEnterBtn');
    if(kicker) kicker.textContent = kind === 'galaxy' ? 'GALAXY MAP' : 'ANDROMEDA TRACE';
    if(title){
      var label = kind === 'galaxy' ? byId('galaxyProgressLabel') : byId('stageProgressLabel');
      title.textContent = label && label.textContent ? label.textContent.trim() : (kind === 'galaxy' ? 'ANDROMEDA TRACE' : 'SANCTUARY');
    }
    if(body){
    ...
    if(enter) enter.textContent = live && live.textContent ? live.textContent.trim() : (kind === 'galaxy' ? 'ENTER ANDROMEDA TRACE' : ('ENTER SANCTUARY ' + activeStage()));
