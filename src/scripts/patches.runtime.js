...
(function(){
  ...
  function setWallet(){var w=$('towerPopupWallet');if(!w)return;var api=window.TowerDefenseGrowth;var shards=Math.max(0,Number(api&&api.getShards?api.getShards():0)||0);var text=shards.toLocaleString('ko-KR');if(w.dataset.v164ShardText===text)return;w.dataset.v164ShardText=text;w.innerHTML='<div class="towerWalletItem shard"><span>성흔 조각</span><b>'+esc(text)+'</b></div>';}
  ...
  function renderTowerDetail(type){
    ...
    var detailKey='tower:'+selectedTowerType+':'+(t.unlocked?'1':'0')+':'+fmt(t.dmg)+':'+fmt(t.range)+':'+fmt(t.cd)+':'+fmt(t.cost);
    if(e.detail.dataset.v164DetailKey===detailKey){requestAnimationFrame(function(){e.detail.scrollLeft=0;});return;}
    e.detail.dataset.v164DetailKey=detailKey;
    if(!t.unlocked){
  ...
  function renderPlateDetail(key){
    ...
    var plateDetailKey='plate:'+p.key;
    if(e.detail.dataset.v164DetailKey===plateDetailKey)return;
    e.detail.dataset.v164DetailKey=plateDetailKey;
